from ZarinaBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from ZarinaBot import DRAGONS
...
async def ZarinaBot_is_admin(chat_id: int):
    status = False
    ZarinaBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if ZarinaBot.id == user.id:
            status = True
            break
    return status
