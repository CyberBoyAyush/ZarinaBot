...
from ZarinaBot.conf import get_int_key, get_str_key

# from ZarinaBot.db.mongo_helpers.nsfw_guard import add_chat, get_all_nsfw_chats, is_chat_in_db, rm_chat
from ZarinaBot.pyrogramee.telethonbasics import is_admin
from ZarinaBot.events import register
from ZarinaBot import MONGO_DB_URI 
from pymongo import MongoClient
from ZarinaBot.modules.sql_extended.nsfw_watch_sql import (
    add_nsfwatch,
    get_all_nsfw_enabled_chat,
    is_nsfwatch_indb,
    rmnsfwatch,
)
from ZarinaBot import telethn as tbot
...
async def ws(event):
    ...
    if nude.is_nude("./nudes.jpg"):
        ...
        hh = sender.id
        final = f"**NSFW DETECTED**\n\n{st}](tg://user?id={hh}) your message contain NSFW content.. So, ZarinaBot deleted the message\n\n **Nsfw Sender - User / Bot :** {st}](tg://user?id={hh})  \n\n`⚔️Automatic Detections Powered By ZarinaBotAI` \n**#GROUP_GUARDIAN** "
...
async def nsfw_watch(client, message):
    ...
    if not hot:
        ...
        await client.send_message(
            message.chat.id,
            f"**NSFW DETECTED**\n\n{hehe}'s message contain NSFW content.. So, ZarinaBot deleted the message\n\n **Nsfw Sender - User / Bot :** `{Escobar}` \n**Chat Title:** `{ctitle}` \n\n`⚔️Automatic Detections Powered By ZarinaBotAI` \n**#GROUP_GUARDIAN** ",
        )
        message.continue_propagation()
"""
...
__help__ = """
<b> Group Guardian: </b>
✪ ZarinaBot can protect your group from NSFW senders, Slag word users and also can force members to use English
